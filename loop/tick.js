...
async function maybeAutoExecute(opps, balances, boxes) {
  ...
  for (const opp of viableOpps) {
    ...
    log(
      `{yellow-fg}🤖 Auto-execução [${executor.label}]: ${pathLabel} (+${opp.result.profitPct.toFixed(3)}%){/}`
    );
    ...
    try {
      ...
      if (res && res.txHash) {
        ...
        logEvent('trade_executed', {
          dex: executor.label,
          path: pathLabel,
          profitPct: opp.result.profitPct,
          profitAbs: opp.result.profitAbs,
...
async function tick(boxes) {
    ...
    for (const v3pool of CONFIG.v3Pools.pools) {
      tasks.push(
        limit(() =>
          taskWithTimeout(
            (async () => {
              const state = await priceEngineV3.fetchPoolState(
                v3pool.address,
                v3pool.tokenA,
                v3pool.tokenB
              );
  ...
  if (hunt && opps && opps.length > 0) {
    try {
      ...
      opps = hunted.opportunities || opps;
      bestOpportunity = hunted.best || opps[0] || null;
      currentOpps = opps;
    } catch (e) {
      logError('hunter', e);
    }
  ...
  if (tickCounter % SNAPSHOT_EVERY_N_TICKS === 0 && opps.length > 0) {
    logEvent('opportunity_snapshot', {
      ...
      crossDexCount: opps.filter(
        (o) => new Set(o.result.steps.map((s) => s.dex)).size > 1
      ).length,
    });
  }
  ...
  if (CONFIG.autoExecute && CONFIG.autoExecute.enabled) {
    await maybeAutoExecute(opps, walletBalances, boxes).catch((e) =>
